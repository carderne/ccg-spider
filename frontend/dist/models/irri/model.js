export default (town, pars) => {
  ...
  const pump_eff = 0.8;
  const kWh_cost = 0.8;
  const crop_production = town.crop_extentmajority*0.5/100*h3size*km2ha*town.cropyield;
  const transp_cost = town.MarketDist * 833 / 1000 * town.crop_extentmajority * pars.tcostperton_km;
  const irrig_cost = (crop_production * CropWaterNeeds * town.WTDmean * pars.pumpenergyint/pump_eff ) * kWh_cost;
